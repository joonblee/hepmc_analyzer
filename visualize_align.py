...
def visualize_event(event, output_file):
    ...
    #    graph.node(f"V{vertex.id}", label=f"Vertex {vertex.id}", shape="circle")
        graph.node(f"V{vertex.id}", label=f"{vertex.id}", shape="circle", width="0.1", height="0.1")

    with graph.subgraph() as proton_subgraph:
        proton_subgraph.attr(rank="same")  # Ensure protons are at the same height

        with graph.subgraph() as final_subgraph:
            ...
            for particle in event.particles:
                ...
                if particle.pid == 2212 and particle.production_vertex.id == 0:
                    ...
                    proton_subgraph.node(start_vertex, label=f"proton", shape="circle", color="blue", style="bold")
                else:
                    start_vertex = f"V{particle.production_vertex.id}" if particle.production_vertex else "Unknown"
                #if particle.pid == 32:
                #    graph.node(end_vertex, label=f"pid={particle.pid} (ID={particle.id})", shape="circle", color="red", style="bold")
                #else:
                ...
                if particle.end_vertex:
                    ...
                    #graph.node(end_vertex, label=f"{particle.pid}", shape="circle")
                    final_subgraph.node(end_vertex, label=f"{particle.pid}", shape="circle")
                    final_particles.add(end_vertex)
    
                if (start_vertex, end_vertex, particle.pid) not in drawn_edges:
                    if highlight:
                        graph.edge(start_vertex, end_vertex, label=f"{particle.pid}", fontcolor="black", fontsize="22", penwidth="5", color="black")
